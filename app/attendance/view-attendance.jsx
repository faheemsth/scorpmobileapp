        ...
        {!!prop?.onReasonClick && (
          <>
            ...
            >
              ...
              <ArrowRight
                style={{ marginStart: 1, width: 12, resizeMode: "contain" }} />
            </TouchableOpacity>
          </>
        )}
...
const ViewAttendanceScreen = () => {
  ...
  useEffect(() => {
    const asyncCall = async () => {
      ...
      const firstDate = new Date(selectedMonth.getFullYear(), selectedMonth.getMonth(), 1);
      const lastDate = new Date(selectedMonth.getFullYear(), selectedMonth.getMonth() + 1, 0);

      const startDate = `1-${firstDate.getMonth() + 1}-${firstDate.getFullYear()}`
      const endDate = `${lastDate.getDate()}-${lastDate.getMonth() + 1}-${lastDate.getFullYear()}`

      const data = await datalayer.attendanceLayer.getAttendanceViewData(startDate, endDate)
  ...
  React.useEffect(() => {
    ...
    if (chips[activeChip].toLowerCase().trim() == "all")
      ...
      setAttendance(
        attendanceData.filter((e) => e.status.toLowerCase() == chips[activeChip].toLowerCase())
      )
  }, [activeChip]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <CalendarList
        ...
        horizontal={true}
        calendarHeight={48}
        onMonthChange={(date) => { setSelectedMonth(new Date(date.dateString)) }}
      />
