...
const ClockInOut = () => {
  ...
  handleCheckInOutClick = () => {
    const updateClockin = async () => {
      const lnp = await datalayer.locationLayer.requestPermissionAndLocation();
      if (!!isClockedIn) {
        if ((await datalayer.attendanceLayer.hasCompletedHours(9)) == false) {
          // show bottom-sheet and request reason
          setRequestEarlyCheckoutReason(true);
          return;
        }
        newValue = !(
          await datalayer.attendanceLayer
            .clockOut(lnp.coords.latitude, lnp.coords.longitude)
            .catch(console.error)
        )["success"];
      } else {
        newValue = (
          await datalayer.attendanceLayer
            .clockIn(lnp.coords.latitude, lnp.coords.longitude)
            .catch(console.error)
        )["success"];
      }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.indexCard}>
        ...
        >
          ...
          />
          <TouchableOpacity activeOpacity={0.9} onPress={handleCheckInOutClick}>
            <LinearGradient
      ...
      <TouchableOpacity
        activeOpacity={0.9}
        onPress={handleViewAttendanceClicked}
      >
        <LinearGradient
          colors={["#167BC4", "#6E7072"]}
          style={styles.linearDeadient}
        >
          <Text style={styles.textA}>View Your Attendance</Text>
        </LinearGradient>
      </TouchableOpacity>
      {requestEarlyCheckoutReason && (
        ...
        >
          <WriteEarlyCheckoutReason onClose={()=>setRequestEarlyCheckoutReason(false)} />
        </View>
      )}
    </SafeAreaView>
  );
